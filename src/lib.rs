...
pub mod prelude {
    pub use super::io::{ReadEndian, ReadPrimitive, WriteEndian};
...
pub trait Endian {
    ...
    /// On a big endian machine, the bytes of this value are reversed.
    #[inline]
    fn from_current_into_little_endian(mut self) -> Self
    where
        Self: Sized,
    {
    ...
    /// On a little endian machine, the bytes of this value are reversed.
    #[inline]
    fn from_current_into_big_endian(mut self) -> Self
    where
        Self: Sized,
    {
    ...
    /// On a big endian machine, the bytes of this value are reversed.
    #[inline]
    fn from_little_endian_into_current(mut self) -> Self
    where
        Self: Sized,
    {
    ...
    /// On a little endian machine, the bytes of this value are reversed.
    #[inline]
    fn from_big_endian_into_current(mut self) -> Self
    where
        Self: Sized,
    {
...
pub mod io {
    ...
    pub trait WriteEndian<T: ?Sized> {
        ...
        fn write_as_native_endian(&mut self, value: &T) -> Result<()> {
            #[cfg(target_endian = "little")]
            {
                self.write_as_little_endian(value)
            }
            #[cfg(target_endian = "big")]
            {
    ...
    pub trait ReadEndian<T: ?Sized> {
        ...
        fn read_from_native_endian_into(&mut self, value: &mut T) -> Result<()> {
            #[cfg(target_endian = "little")]
            {
                self.read_from_little_endian_into(value)
            }
            #[cfg(target_endian = "big")]
            {
                self.read_from_big_endian_into(value)
            }
        }
            ...
            T: Sized + Default,
        {
            #[cfg(target_endian = "little")]
            {
                self.read_from_little_endian()
            }
            #[cfg(target_endian = "big")]
            {
                self.read_from_big_endian()
            }
        }
    }
